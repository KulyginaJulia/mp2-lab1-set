...
{
	BitLen = len; 
	if (BitLen < 0){ int exception = 1; throw exception;}
		else {MemLen = (len + 15) >> 4;
	...
	if (pMem != NULL) 
		...
		cout << "Exception is negative length" << endl;
		//break;
		//exit(1);
	}
	catch (const char *){
...
{
	//if ((n < 0)||(n > 9)){ int exception = 1; throw exception;}
	if ((n < 0)||(n > BitLen)){ int exception = 1; throw exception;}
		else {
	if ( (n > -1) && (n < BitLen) )
